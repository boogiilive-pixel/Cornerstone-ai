...
const teamMembers = [
  { id: 1, name: "Б.Мөнхбат", role: "Ахлах борлуулагч", email: "munkhbat@cornerstone.mn", status: "Active", image: "B" },
  { id: 2, name: "Д.Оюунчимэг", role: "Санхүүгийн менежер", email: "oyunchimeg@cornerstone.mn", status: "Active", image: "O" },
  { id: 3, name: "Г.Төмөрбаатар", role: "Маркетингийн мэргэжилтэн", email: "tumurbaatar@cornerstone.mn", status: "Away", image: "T" },
  { id: 4, name: "Н.Сарантуяа", role: "Хүний нөөц", email: "sarantuya@cornerstone.mn", status: "Active", image: "S" },
  { id: 5, name: "Э.Болормаа", role: "Төслийн менежер", email: "bolormaa@cornerstone.mn", status: "Offline", image: "E" },
];

const transactions = [
  { id: "TRX-9821", date: "2024.11.14", client: "Говь ХК", amount: "₮12,400,000", status: "Амжилттай" },
  { id: "TRX-9820", date: "2024.11.14", client: "АПУ ХК", amount: "₮8,200,000", status: "Хүлээгдэж буй" },
  { id: "TRX-9819", date: "2024.11.13", client: "М-Си-Эс", amount: "₮45,000,000", status: "Амжилттай" },
  { id: "TRX-9818", date: "2024.11.13", client: "Юнител", amount: "₮2,100,000", status: "Цуцлагдсан" },
];
      ...
      <main className="flex-1 min-h-screen relative overflow-y-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl md:text-2xl font-bold mb-1">
                {activeNav === "Overview" && "Ерөнхий тойм"}
                {activeNav === "Sales" && "Борлуулалт"}
                {activeNav === "Finance" && "Санхүү"}
                {activeNav === "Team" && "Ажилтнууд"}
                {activeNav === "Reports" && "Тайлан"}
                {activeNav === "Settings" && "Тохиргоо"}
              </h2>
        ...
        <div className="px-4 md:px-8 pb-12 space-y-6 md:space-y-8">
          ...
          {activeNav === "Sales" && (
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  { label: "Өнөөдрийн борлуулалт", value: "₮12.4M", change: "+5.2%", icon: ShoppingBag },
                  { label: "Дундаж чек", value: "₮245,000", change: "+1.8%", icon: TrendingUp },
                  { label: "Хөрвүүлэлт", value: "3.2%", change: "+0.4%", icon: BarChart3 },
                ].map((stat) => (
                  <div key={stat.label} className="bg-[#111827]/60 p-6 rounded-2xl border border-white/5">
                    <div className="flex items-center justify-between mb-4">
                      <stat.icon className="w-5 h-5 text-[#C49A3C]" />
                      <span className="text-[10px] font-bold text-[#10B981]">{stat.change} ↑</span>
                    </div>
                    <p className="text-2xl font-bold">{stat.value}</p>
                    <p className="text-xs text-[#6B7A99] uppercase tracking-wider">{stat.label}</p>
                  </div>
                ))}
              </div>
              <div className="bg-[#111827]/60 p-8 rounded-2xl border border-white/5">
                <h3 className="text-lg font-bold mb-6">Сүүлийн гүйлгээнүүд</h3>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="text-left border-b border-white/5">
                        <th className="pb-4 text-[10px] uppercase tracking-widest text-[#6B7A99]">ID</th>
                        <th className="pb-4 text-[10px] uppercase tracking-widest text-[#6B7A99]">Огноо</th>
                        <th className="pb-4 text-[10px] uppercase tracking-widest text-[#6B7A99]">Харилцагч</th>
                        <th className="pb-4 text-[10px] uppercase tracking-widest text-[#6B7A99]">Дүн</th>
                        <th className="pb-4 text-[10px] uppercase tracking-widest text-[#6B7A99]">Төлөв</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                      {transactions.map((trx) => (
                        <tr key={trx.id} className="hover:bg-white/5 transition-colors">
                          <td className="py-4 text-sm font-mono text-[#C49A3C]">{trx.id}</td>
                          <td className="py-4 text-sm text-[#6B7A99]">{trx.date}</td>
                          <td className="py-4 text-sm font-medium">{trx.client}</td>
                          <td className="py-4 text-sm font-bold">{trx.amount}</td>
                          <td className="py-4">
                            <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${
                              trx.status === "Амжилттай" ? "text-[#10B981] bg-[#10B981]/10" : 
                              trx.status === "Хүлээгдэж буй" ? "text-[#C49A3C] bg-[#C49A3C]/10" : 
                              "text-[#EF4444] bg-[#EF4444]/10"
                            }`}>
                              {trx.status}
                            </span>
          ...
          {activeNav === "Finance" && (
            <div className="space-y-8">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-[#111827]/60 p-8 rounded-2xl border border-white/5">
                  <h3 className="text-lg font-bold mb-6">Мөнгөн урсгал</h3>
                  <div className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={salesData}>
                        <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} />
                        <XAxis dataKey="name" stroke="#6B7A99" fontSize={10} />
                        <YAxis stroke="#6B7A99" fontSize={10} />
                        <Tooltip contentStyle={{ backgroundColor: "#0D1526", border: "1px solid rgba(255,255,255,0.1)" }} />
                        <Bar dataKey="income" fill="#C49A3C" radius={[4, 4, 0, 0]} />
                ...
                <div className="bg-[#111827]/60 p-8 rounded-2xl border border-white/5">
                  <h3 className="text-lg font-bold mb-6">Зардлын бүтэц</h3>
                  <div className="space-y-6">
                    {financialSummary.map((item) => (
                      <div key={item.category} className="flex items-center justify-between p-4 bg-white/5 rounded-xl">
                        <div className="flex items-center gap-4">
          ...
          {activeNav === "Team" && (
            <div className="bg-[#111827]/60 p-8 rounded-2xl border border-white/5">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-lg font-bold">Багийн гишүүд</h3>
                <button className="bg-[#C49A3C] text-stealth-black px-4 py-2 rounded-lg text-xs font-bold">Шинэ ажилтан +</button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {teamMembers.map((member) => (
                  <div key={member.id} className="p-6 bg-white/5 rounded-2xl border border-white/5 hover:border-[#C49A3C]/30 transition-all group">
                    <div className="flex items-center gap-4 mb-4">
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#C49A3C] to-[#8B6E2A] flex items-center justify-center font-bold text-stealth-black text-lg">
                        {member.image}
                      </div>
                      <div>
                    ...
                    <div className="space-y-2 mb-4">
                      <p className="text-[10px] text-[#6B7A99] uppercase tracking-widest">Имэйл</p>
                      <p className="text-xs">{member.email}</p>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${
                        member.status === "Active" ? "text-[#10B981] bg-[#10B981]/10" : 
                        member.status === "Away" ? "text-[#C49A3C] bg-[#C49A3C]/10" : 
                        "text-[#6B7A99] bg-white/5"
                      }`}>
                        ● {member.status}
                      </span>
                      <button className="p-2 text-[#6B7A99] hover:text-white"><MoreVertical className="w-4 h-4" /></button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeNav === "Reports" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                { title: "Жилийн тайлан 2023", size: "4.2 MB", type: "PDF" },
                { title: "Борлуулалтын дүн шинжилгээ Q3", size: "1.8 MB", type: "XLSX" },
                { title: "Ажилтны гүйцэтгэлийн тайлан", size: "2.5 MB", type: "PDF" },
                { title: "Санхүүгийн аудит 2024", size: "5.1 MB", type: "PDF" },
              ].map((report) => (
                <div key={report.title} className="p-6 bg-[#111827]/60 rounded-2xl border border-white/5 flex items-center justify-between group hover:bg-white/5 transition-all">
                  <div className="flex items-center gap-4">
                    <div className="p-3 bg-[#C49A3C]/10 rounded-xl text-[#C49A3C]">
                      <BarChart3 className="w-6 h-6" />
                    </div>
                    <div>
                      <h4 className="font-bold text-sm">{report.title}</h4>
                      <p className="text-[10px] text-[#6B7A99] uppercase tracking-widest">{report.size} • {report.type}</p>
                    </div>
                  </div>
                  <button className="text-xs font-bold text-[#C49A3C] hover:underline">Татах ↓</button>
                </div>
              ))}
            </div>
          )}
          ...
          {activeNav === "Settings" && (
            <div className="max-w-2xl space-y-8">
              <div className="bg-[#111827]/60 p-8 rounded-2xl border border-white/5">
                <h3 className="text-lg font-bold mb-6">Профайл тохиргоо</h3>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="text-[10px] text-[#6B7A99] uppercase tracking-widest">Нэр</label>
                      <input type="text" defaultValue="Boogii" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-sm focus:border-[#C49A3C] outline-none transition-all" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] text-[#6B7A99] uppercase tracking-widest">Имэйл</label>
                      <input type="email" defaultValue="admin@cornerstone.mn" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-sm focus:border-[#C49A3C] outline-none transition-all" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] text-[#6B7A99] uppercase tracking-widest">Хэл</label>
                    <select className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-sm focus:border-[#C49A3C] outline-none transition-all appearance-none">
              ...
              <div className="bg-[#111827]/60 p-8 rounded-2xl border border-white/5">
                <h3 className="text-lg font-bold mb-6">Мэдэгдэл</h3>
                <div className="space-y-4">
                  {[
                    "Имэйл мэдэгдэл авах",
                    "Борлуулалтын мэдээ авах",
                    "Системийн шинэчлэлт",
                  ].map((item) => (
